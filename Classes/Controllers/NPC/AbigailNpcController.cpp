...
AbigailNpcController::AbigailNpcController(
    ...
    : map_(map),
      ...
      dialogue_(dialogue) {
  // 基于地图中心和偏移量，计算 Abigail 的出生位置，并尝试微调避免与障碍重叠。
  ...
  if (!npc_) return;
  auto sprite = cocos2d::Sprite::create(framePath("Abigail", kDirDown, 1));
  if (!sprite) return;
  if (sprite->getTexture()) sprite->getTexture()->setAliasTexParameters();
  float offset_x = 6.0f * tile_;
  float offset_y = 6.5f * tile_;
  ...
  // 把 Abigail 精灵添加到地图图层，并记录为后续巡逻的基准点。
  sprite->setPosition(pos);
  map_->addActorToMap(sprite, 22);
  sprite_ = sprite;
  patrol_base_pos_ = pos;

  walk_down_ = buildWalkAnim("Abigail", kDirDown);
  walk_up_ = buildWalkAnim("Abigail", kDirUp);
  walk_left_ = buildWalkAnim("Abigail", kDirLeft);
  walk_right_ = buildWalkAnim("Abigail", kDirRight);
...
void AbigailNpcController::update(const cocos2d::Vec2& player_pos) {
  if (!ui_ || !map_) return;
  // 依据地图环境层高度调整绘制顺序，让 NPC 正确被树木/建筑遮挡。
...
// 为 Abigail 构建一个循环巡逻路线，遇到碰撞会由地图帮助调整目的地。
void AbigailNpcController::startPatrol() {
...
void AbigailNpcController::playWalk(int dir) {
  if (!sprite_) return;
...
void AbigailNpcController::setStanding(int dir) {
  if (!sprite_) return;
  sprite_->stopActionByTag(kWalkActionTag);
  facing_dir_ = dir;
  std::string path = framePath("Abigail", dir, 1);
...
// 右键点击 Abigail 精灵时，打开对应的社交面板。
bool AbigailNpcController::handleRightClick(cocos2d::EventMouse* e) {
