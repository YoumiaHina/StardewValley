...
|    冯泉睿    | 2353365 |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|    阮文涛    | 2350757 |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|    张国雄    | 2352875 | **社交系统、节日系统、钓鱼系统、音乐系统及一些 UI、场景显示的优化工作等** `FestivalController`：节日开启/关闭的状态驱动与节日流程控制 `FishingController`：钓鱼小游戏流程与节日钓鱼等玩法接入 `AudioManager`：BGM/音效播放、切歌与防重复触发等音频管理 `NpcControllerBase` / 各 NPC 控制器：对话、赠礼与社交入口的交互逻辑 `NpcSocialPanelUI` / `DialogueUI`：社交面板与对话 UI 的展示与交互 `UIController`：HUD/背包/面板聚合管理画面的优化<br /> `TownScene` / `MainMenuScene` / `SplashScene`：场景显示与切换流程的体验优化 |
...
- 资源与内存：多数 Cocos 节点生命周期交由场景树引用计数管理；同时在 UI/怪物等模块析构中主动解绑与清理。少量控制器仍以 `new` 创建且释放链路分散，后续可进一步收敛为统一 owner 或 `std::unique_ptr` 托管以提升稳定性。
...
- 玩家确定外观并进入游戏后，对应的外观将被记录在存档中，后续加载存档可读取外观并正确显示。
  ...
  ![1766832164924](image/README/1766832164924.png)
...
- 商店面板支持分类与分页，便于快速浏览与交易。
  ![1766832217971](image/README/1766832217971.png)
...
- 睡觉进入下一天会触发作物推进，并重置当天“已浇水”的状态。
  ...
  ![1766832527941](image/README/1766832527941.png)
...
- 每天睡觉后结算成长与产物；不在农场时产物会延迟到回到农场后出现为掉落物。
  ...
  ![1766833387189](image/README/1766833387189.png)
...
- 掉落物可在地面拾取进入背包，用于后续合成、升级与交易。
  ...
  ![1766833671044](image/README/1766833671044.png)
...
- 采矿会持续消耗精力，需要在矿洞中合理安排路线，或返回农场休息补给。
  ...
  ![1766833816708](image/README/1766833816708.png)
...
- 战斗过程中需要同时关注生命值与精力，合理使用食物/药品，并在必要时撤退返回农场存档。
  ...
  ![1766832682176](image/README/1766832682176.png)
...
- 在农场中，玩家使用斧头可移除空箱子（若箱子内有物品则无法移除），使用镐子可以移除空闲状态的熔炉。
  ...
  ![1766832735323](image/README/1766832735323.png)
...
- 合成结果会即时反馈到背包与 UI 显示，便于连续制作与资源规划。
  ...
  ![1766832773873](image/README/1766832773873.png)
...
- 满级工具可以一次性对面前的 3*3 范围进行操作（使用Ctrl+左键），也可与低级工具一样对单个格子进行操作（左键）。
  ...
  ![1766833543550](image/README/1766833543550.png)
...
- 技能加成在系统结算时生效，并通过 UI 面板可随时查看当前收益与升级进度。
  ...
  ![1766832792564](image/README/1766832792564.png)
...
- 若玩家晚于0点睡觉，会触发晕倒，扣除金币并自动进入第二天，在Room的床上醒来
  ...
  ![1766832835766](image/README/1766832835766.png)
...
- 季节会自动循环，每个季节结束后会触发下一个季节的开启。
  ...
  ![1766832882727](image/README/1766832882727.png)
...
- 节日会在特定日期开启，城镇/海滩出现活动布置与特殊交互内容。
  ...
  ![1766832946583](image/README/1766832946583.png)
...
- 特定节日提供“节日钓鱼”玩法与限定鱼种奖励。
  ...
  ![1766833003103](image/README/1766833003103.png)
